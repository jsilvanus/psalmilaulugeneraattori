...
import type { Chord } from '../src/tone/chordTypes.js';
...
const cola: PitchedChordColon[] = [
  {
    role: 'mediant',
    syllables: [
      { text: 'Di', chords: [chord(4, 2, -1, -5)], isWordStart: true },
      { text: 'xit', chords: [chord(3, 1, -2, -6)], isWordStart: false },
    ],
  },
  {
    role: 'termination',
    syllables: [{ text: 'se', chords: [chord(2, 0, -3, -7)], isWordStart: true }],
  },
];
...
describe('emitAbcChordal', () => {
  ...
  it('throws for a chord voice outside the supported ABC pitch range', () => {
    const outOfRange: PitchedChordColon[] = [
      {
        role: 'termination',
        syllables: [{ text: 'x', chords: [chord(50, 0, 0, 0)], isWordStart: true }],
      },
    ];
